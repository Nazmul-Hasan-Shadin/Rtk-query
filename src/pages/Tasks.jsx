...
const Tasks = () => {
  ...
  const pendingTasks = tasks?.message?.filter((item) => item.status == "pending");
  const runningTasks = tasks?.message.filter((item) => item.status == "running");
  const doneTasks = tasks?.message.filter((item) => item.status == "done");
